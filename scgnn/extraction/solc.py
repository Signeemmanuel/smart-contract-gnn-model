...
def solc_for_file(path: str | Path, binaries: dict[str, str], head_bytes: int = 4096) -> str | None:
    """Resolve the solc binary for one contract file by reading its pragma.

    Returns a binary path from ``binaries`` (typically ``installed_solc_binaries``)
    or None when the file is unreadable, has no pragma, or no matching solc is
    installed — callers then skip the contract or fall back to PATH ``solc``.
    """
    try:
